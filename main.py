...
class JiraAPI:
    ...
    def getAttachmentUrl(self, attachment_id):
        """Obter URL direta do anexo com credenciais"""
        config = load_config()
        return {
            "success": True, 
            "url": f"{jira_url()}/rest/api/3/attachment/content/{attachment_id}",
            "auth": {
                "email": config.get("jiraEmail", ""),
                "apiToken": config.get("jiraApiToken", "")
            }
        }
    
    def getAttachmentContent(self, attachment_id):
        """Baixar conteúdo do anexo e retornar como base64"""
        import base64
        try:
            # Primeiro, obter metadados do anexo para saber o tipo correto
            meta_r = requests.get(
                f"{jira_url()}/rest/api/3/attachment/{attachment_id}",
                auth=get_auth(),
                timeout=30
            )
            ...
            if meta_r.status_code == 200:
                meta = meta_r.json()
                expected_mime = meta.get('mimeType')
                print(f"[api] Attachment metadata: {meta.get('filename')}, type={expected_mime}")
            ...
            if r.status_code != 200:
                print(f"[api] Attachment error: status {r.status_code}")
                return {"success": False, "error": f"HTTP {r.status_code}"}
            
            content_type = r.headers.get('Content-Type', 'application/octet-stream')
            
            # Verificar se recebemos HTML em vez do arquivo (erro de autenticação/SSO)
            if 'text/html' in content_type and expected_mime and 'text/html' not in expected_mime:
                print(f"[api] Attachment error: received HTML instead of {expected_mime}")
                return {"success": False, "error": "Erro de autenticação. Verifique suas credenciais do Jira."}
            
            # Usar o tipo esperado se disponível
            final_mime = expected_mime or content_type
            data_base64 = base64.b64encode(r.content).decode('utf-8')
            
            print(f"[api] Attachment {attachment_id}: {len(r.content)} bytes, type={final_mime}")
            return {
                "success": True,
                "data": data_base64,
                "mimeType": final_mime,
            ...
            print(f"[api] getAttachmentContent error: {e}")
            return {"success": False, "error": str(e)}
